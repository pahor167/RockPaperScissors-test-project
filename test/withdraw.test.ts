...
describe("RockPaperScissors withdraw tests", function () {
  ...
  it("Withdraw All sufficient balance", async function () {
    ...
    const approveTx = await testErc
      .connect(player)
      .approve(rockPaperScissors.address, depositAmount);
    await approveTx.wait();

    const depositTx = await rockPaperScissors
      .connect(player)
  ...
  it("Withdraw All insufficient balance", async function () {
    ...
    await expect(
      rockPaperScissors.connect(player).widthdrawAll()
    ).to.be.revertedWith("balance > 0");
  });
  ...
  it("Withdraw part of balance", async function () {
    ...
    ).to.equal(depositAmount - amountToWidthdraw);
  });
  ...
  it("Withdraw more than balance", async function () {
    ...
    await expect(
      rockPaperScissors.widthdraw(depositAmount + 1)
    ).to.be.revertedWith("balance >= amount");
