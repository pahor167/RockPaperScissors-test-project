...
describe("RockPaperScissors deposit tests", function () {
  ...
  it("Deposit below bet amount", async function () {
    ...
    await expect(
      rockPaperScissors.connect(player).deposit(depositAmount)
    ).to.be.revertedWith("amount >= _betAmount");
  ...
  it("Deposit bet amount", async function () {
    ...
    expect(
      (
        await rockPaperScissors.connect(player)._balances(player.address)
      ).toNumber()
