...
import { PlayMoveEnum } from "./Utils/enums";
...
describe("RockPaperScissors play tests", function () {
  ...
  it("Play insufficient balance", async function () {
    expect(
      (
        await rockPaperScissors.connect(playerA).balanceOf(playerA.address)
      ).toNumber()
    ).to.equal(0);
    const myPlayHashed = hashMove("secretPhase", PlayMoveEnum.Paper);
    await expect(
      rockPaperScissors.connect(playerA).startPublicGame(myPlayHashed as string)
    ).to.be.revertedWith("balance >= betAmount");
  });
  ...
  it("Play - return money to playerA after deadline - playerB didn't respond ", async function () {
    ...
    await testErc
      .connect(playerA)
      .approve(rockPaperScissors.address, MAX_UINT256.toString());
    const depositPlayerATx = await rockPaperScissors
      .connect(playerA)
      .deposit(depositAmount);
    const depositPlayerATxWait = await depositPlayerATx.wait();
    expectEvent(
      depositPlayerATxWait.events,
      "Deposit",
      playerA.address,
      depositAmount
    );
    ...
    await testErc
      .connect(playerB)
      .approve(rockPaperScissors.address, MAX_UINT256.toString());
    const depositplayerBTx = await rockPaperScissors
      .connect(playerB)
      .deposit(depositAmount);
    const depositplayerBTxWait = await depositplayerBTx.wait();
    expectEvent(
      depositplayerBTxWait.events,
      "Deposit",
      playerB.address,
      depositAmount
    );
    ...
    expect(
      (
        await rockPaperScissors.connect(playerB).balanceOf(playerB.address)
      ).toNumber()
    ).to.equal(depositAmount);
    ...
    const playerAPlayTx = await rockPaperScissors
      .connect(playerA)
      .startPrivateGame(playerAMoveHashed as string, playerB.address);
    const playerAPlayTxWait = await playerAPlayTx.wait();

    const event = expectEvent(
      playerAPlayTxWait.events,
      "GameStarted",
      playerA.address,
      playerB.address
    );
    ...
    const revealTx = await rockPaperScissors
      .connect(playerC)
      .revealAfterDeadline(gameId);
    const revealTxWait = await revealTx.wait();

    expectEvent(
      revealTxWait.events,
      "GameRevealedAfterDeadline",
      playerA.address,
      playerB.address,
      gameId
    );
  ...
  it("Play - try to reavealAfterDeadline before deadline ", async function () {
    ...
    await expect(
      rockPaperScissors.revealAfterDeadline(gameId)
    ).to.be.revertedWith("game.deadline < block.timestamp");
  });
  ...
  it("Play - return money to playerA after deadline - playerB responded ", async function () {
    ...
    await testErc
      .connect(playerA)
      .approve(rockPaperScissors.address, depositAmount);
    const depositPlayerATx = await rockPaperScissors
      .connect(playerA)
      .deposit(depositAmount);
    const depositPlayerATxWait = await depositPlayerATx.wait();
    ...
    const oponnentPlayTx = await rockPaperScissors
      .connect(playerB)
      .respond(gameId, PlayMoveEnum.Paper);
    const oponnentPlayTxWait = await oponnentPlayTx.wait();
    expectEvent(
      oponnentPlayTxWait.events,
      "GameResponded",
      playerA.address,
      playerB.address,
      gameId,
  ...
  describe("play scenarios", () => {
    const tests = [
      {
        args: {
          playerAMove: PlayMoveEnum.Rock,
          playerBMove: PlayMoveEnum.Rock,
        },
        expected: {
          playerABalance: betAmount,
          playerBBalance: betAmount,
          winner: () => 0,
        },
      },
      {
        args: {
          playerAMove: PlayMoveEnum.Rock,
          playerBMove: PlayMoveEnum.Paper,
        },
        expected: {
          playerABalance: 0,
          playerBBalance: betAmount * 2,
      ...
      },
      {
        args: {
          playerAMove: PlayMoveEnum.Rock,
          playerBMove: PlayMoveEnum.Scissors,
        },
        expected: {
          playerABalance: betAmount * 2,
          playerBBalance: 0,
      ...
      {
        args: {
          playerAMove: PlayMoveEnum.Paper,
          playerBMove: PlayMoveEnum.Rock,
        },
        expected: {
          playerABalance: betAmount * 2,
      ...
      {
        args: {
          playerAMove: PlayMoveEnum.Scissors,
          playerBMove: PlayMoveEnum.Rock,
        },
        expected: {
          playerABalance: 0,
    ...
    tests.forEach(({ args, expected }) => {
      it(`Play scenario PlayerA: ${PlayMoveEnum[args.playerAMove]} playerB: ${
        PlayMoveEnum[args.playerBMove]
      }; PlayerA expected balance: ${
        expected.playerABalance
      } playerB expected balance: ${expected.playerBBalance}`, async () => {
        const depositAmount = betAmount;
        ...
        const oponnentPlayTx = await rockPaperScissors
          .connect(playerB)
          .respond(gameId, args.playerBMove);
        const oponnentPlayTxWait = await oponnentPlayTx.wait();
        expectEvent(
          oponnentPlayTxWait.events,
          "GameResponded",
        ...
        const secretPhaseInBytes32 = ethers.utils.hexZeroPad(
          web3.utils.fromUtf8(secretPhase),
          32
        );
        const revealTx = await rockPaperScissors
          .connect(playerA)
          .reveal(secretPhaseInBytes32, gameId);
        const revealTxWait = await revealTx.wait();
  ...
  it(`Game with 0 bet amount`, async () => {
    ...
    const oponnentPlayTx = await rockPaperScissors
      .connect(playerB)
      .respond(gameId, PlayMoveEnum.Scissors);
    const oponnentPlayTxWait = await oponnentPlayTx.wait();
    expectEvent(
      oponnentPlayTxWait.events,
      "GameResponded",
  ...
  it(`3 players, public game, reuse won balance`, async () => {
    ...
    await testErc
      .connect(playerC)
      .approve(rockPaperScissors.address, depositAmount);
    const depositplayerCTx = await rockPaperScissors
      .connect(playerC)
      .deposit(depositAmount);
    const depositplayerCTxWait = await depositplayerCTx.wait();
    expectEvent(
      depositplayerCTxWait.events,
      "Deposit",
      playerC.address,
      depositAmount
    );
    ...
    const playerAPlayTx = await rockPaperScissors
      .connect(playerA)
      .startPublicGame(playerAMoveHashed as string);
    const playerAPlayTxWait = await playerAPlayTx.wait();
    ...
    const withdrawTx = await rockPaperScissors
      .connect(playerA)
      .widthdraw(betAmount);

    const secretPhaseWithC = "mysecretWithC";
    const playerAMoveHashedWithC = hashMove(
      secretPhaseWithC,
      PlayMoveEnum.Paper
    );

    const playerAPlayTxWithC = await rockPaperScissors
      .connect(playerA)
      .startPublicGame(playerAMoveHashedWithC as string);
    const playerAPlayTxWaitWithC = await playerAPlayTxWithC.wait();

    const eventWithC = expectEvent(
      playerAPlayTxWaitWithC.events,
      "GameStarted",
      playerA.address,
      0
    );
    ...
    const oponnentPlayCTx = await rockPaperScissors
      .connect(playerC)
      .respond(gameIdWithC, PlayMoveEnum.Scissors);
    const oponnentPlayCTxWait = await oponnentPlayCTx.wait();
    expectEvent(
      oponnentPlayCTxWait.events,
      "GameResponded",
      playerA.address,
      playerC.address,
      gameIdWithC,
    ...
    const secretPhaseInBytes32WithC = ethers.utils.hexZeroPad(
      web3.utils.fromUtf8(secretPhaseWithC),
      32
    );
    const revealWithCTx = await rockPaperScissors
      .connect(playerA)
      .reveal(secretPhaseInBytes32WithC, gameIdWithC);
    const revealWithCTxWait = await revealWithCTx.wait();

    expectEvent(
      revealWithCTxWait.events,
      "GameRevealed",
      playerA.address,
      playerC.address,
      gameIdWithC,
