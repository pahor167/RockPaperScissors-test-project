...
contract RockPaperScissors is Ownable {
    ...
    event GameRevealedAfterDeadline(
        address playerA,
        address playerB,
        bytes32 gameId
    );
    ...
    /** @dev
     * @param gameId GameId can be retrieved from GameStarted event.
    ...
    /** @dev 
     * This method can be called only by player that knows secret phase for particular game (usually player that started the game - playerA).
    ...
    function reveal(bytes32 secretPhrase, bytes32 gameId) public {
        ...
        require(
            game.playerBMove != GameMove.None,
            "game.playerBMove != GameMove.None"
        );
        require(
            game.gameState == GameState.Responded,
    ...
    function setGameDeadlineInSeconds(uint32 gameDeadlineInSeconds)
        public
        onlyOwner
    {
    ...
    function decodeMoveAndEvaluate(bytes32 secretPhrase, Game storage game)
        private
        returns (address)
    {
    ...
    function updateWageredBalances(
        address playerA,
        address playerB,
        uint256 betAmount
    ) private {
        ...
        require(
            wageredBalanceSender >= betAmount,
            "wageredBalanceSender >= betAmount"
        );
        unchecked {
        ...
        require(
            playerBWageredBalance >= betAmount,
            "playerBWageredBalance >= betAmount"
        );
        unchecked {
    ...
    function startGame(bytes32 hashedMove, address playerB) private {
        ...
        bytes32 gameId = keccak256(
            abi.encodePacked(msg.sender, hashedMove, block.number)
        );
        Game storage game = _games[gameId];
